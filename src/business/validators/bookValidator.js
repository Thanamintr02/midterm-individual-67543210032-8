...
class BookValidator {
   ...
   const bookValidator = {
    validate: (data) => {
        if (!data.title || !data.author || !data.isbn) throw new Error('Missing required fields');
        if (!/^(97[89])?\d{9}[\dXx]$/.test(data.isbn.replace(/-/g, ''))) throw new Error('Invalid ISBN format');
    }
};
